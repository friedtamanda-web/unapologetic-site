#!/usr/bin/env python3
import re, json, base64, os, html

SRC = "from-the-field.html"
BASE = "https://theunapologeticleader.com"
os.makedirs("lines", exist_ok=True)
os.makedirs("img/lines", exist_ok=True)

h = open(SRC).read()
m = re.search(r'DATA\s*=\s*(\[.*?\])\s*;', h, re.S)
data = json.loads(m.group(1))
print("parsed items:", len(data), "| keys:", list(data[0].keys()))

SLUGS = {
 3:"set-free", 17:"rest-is-integrity", 23:"let-the-circus-be-theirs",
 28:"i-am-the-pilot", 30:"loving-me-first", 31:"nothing-meant-to-be-mine",
 32:"content-built-empires", 35:"a-purpose-everywhere", 37:"too-much-love",
 38:"be-the-lighthouse", 44:"exactly-where-you-need-to-be", 45:"the-light-never-moved",
}

def img_key(item):
    for k in ("warm","soul","light","card","dark"):
        if k in item and isinstance(item[k],str) and len(item[k])>200: return k
    return None
...
html{-webkit-font-smoothing:antialiased;background:#fff;scroll-behavior:smooth}
...
.ni .lk{font-family:var(--mono);font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:var(--ink-soft)}
.ni .lk:hover{color:var(--pink)}
...
.more{text-align:center;margin:26px auto 0}
.more a{font-family:var(--mono);font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:var(--gray);border-bottom:1px solid var(--line)}
...
.foot-links a{font-family:var(--mono);font-size:.74rem;letter-spacing:.08em;text-transform:uppercase;color:var(--gray)}
.foot-links a:hover{color:var(--pink)}
.toast{position:fixed;left:50%;bottom:30px;transform:translateX(-50%) translateY(20px);background:var(--ink);color:#fff;padding:12px 22px;border-radius:30px;font-size:14px;opacity:0;pointer-events:none;transition:all .25s;z-index:50}
...
<link rel="canonical" href="{base}/lines/{slug}.html">
...
<meta property="og:url" content="{base}/lines/{slug}.html"><meta property="og:image" content="{base}/img/lines/{slug}.png">
<meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="{q}"><meta name="twitter:image" content="{base}/img/lines/{slug}.png">
...
<script type="application/ld+json">{{"@context":"https://schema.org","@type":"Quotation","text":{qj},"spokenByCharacter":{{"@type":"Person","name":"Amanda Friedt"}},"creator":{{"@type":"Person","name":"Amanda Friedt"}},"isPartOf":{{"@type":"CollectionPage","name":"Lines to Carry","url":"{base}/from-the-field.html"}},"image":"{base}/img/lines/{slug}.png","url":"{base}/lines/{slug}.html"}}</script>
{ga}
{css}
</head><body>
...
<header><div class="wrap">
  <div class="kick">A line to carry</div>
  <h1>{q}</h1>
  <div class="rule"></div>
</div></header>
<div class="wrap">
  <div class="cardwrap"><img src="../img/lines/{slug}.png" alt="{q}"></div>
  <div class="cta-row">
    <button class="btn btn-primary" onclick="shareLine()">Share this</button>
    <a class="btn btn-text" href="https://theunapologeticleader.substack.com/?utm_source=lines&utm_medium={slug}">Walk with us</a>
  </div>
  <div class="more"><a href="../from-the-field.html">See all the lines →</a></div>
</div>
<footer><div class="wrap">
  <div>© 2026 The Unapologetic Leader · Detroit · Her words, cleaned for legibility, never rewritten.</div>
  <div class="foot-links"><a href="../index.html">Home</a><a href="../start-here.html">Start Here</a><a href="../from-the-field.html">From the Field</a><a href="https://theunapologeticleader.substack.com/">Subscribe</a></div>
</div></footer>
...
async function shareLine(){{
  var url=location.href, text={qj}+'  \\u2014 The Unapologetic Leader  theunapologeticleader.com';
  try{{var r=await fetch('../img/lines/{slug}.png');var b=await r.blob();var f=new File([b],'{slug}.png',{{type:'image/png'}});
...
made=[]
for item in data:
    cid=item.get("id"); slug=SLUGS.get(cid)
    if not slug:
        print("no slug for id",cid); continue
    text=item.get("text","").strip()
    k=img_key(item)
    if not k:
        print("no image for",cid); continue
    png=base64.b64decode(item[k])
    open(f"img/lines/{slug}.png","wb").write(png)
    q=html.escape(text)
    out=PAGE.format(q=q, qj=json.dumps(text), slug=slug, base=BASE, ga=GA, css=CSS)
    open(f"lines/{slug}.html","w").write(out)
    made.append((cid,slug,text))
print("generated",len(made),"permalink pages + images")
for cid,slug,text in made: print(f"  /lines/{slug}.html  <- {text}")
# write id->slug map for wiring
open("_lines_map.json","w").write(json.dumps({str(c):s for c,s,_ in made}))
